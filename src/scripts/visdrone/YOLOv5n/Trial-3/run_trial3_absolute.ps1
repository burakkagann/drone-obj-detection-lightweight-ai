# YOLOv5n Trial-3 Training Script - Absolute Paths Version
...
$CONFIG = @{
    ...
    TRIAL_NAME = "trial3"
    DATASET = "visdrone"
    CONFIG_PATH = "$RepoRoot\config\visdrone\yolov5n_v1\yolov5n_visdrone_config.yaml"
    MODEL_CONFIG_PATH = "$RepoRoot\src\models\YOLOv5\models\yolov5n.yaml"
    HYP_CONFIG_PATH = "$RepoRoot\config\visdrone\yolov5n_v1\hyp_visdrone_trial3.yaml"
    DATASET_PATH = "$RepoRoot\data\my_dataset\visdrone"
    YOLOV5_PATH = "$RepoRoot\src\models\YOLOv5"
    BATCH_SIZE = 20
    EPOCHS = 100
    IMAGE_SIZE = 640
    SAVE_PERIOD = 5
...
function Test-Prerequisites {
    ...
    Write-Host "Using Trial-3 hyperparameters from: $($CONFIG.HYP_CONFIG_PATH)"
    
    $requiredPaths = @(
        "$($CONFIG.DATASET_PATH)\train\images",
        "$($CONFIG.DATASET_PATH)\val\images", 
        "$($CONFIG.DATASET_PATH)\train\labels",
...
function Train-Trial3-YOLOv5 {
    ...
    if (-not (Test-Path $CONFIG.YOLOV5_PATH)) {
        throw "YOLOv5 directory not found at: $($CONFIG.YOLOV5_PATH)"
    }
    
    Write-Host "Navigating to YOLOv5 directory: $($CONFIG.YOLOV5_PATH)"
    Push-Location $CONFIG.YOLOV5_PATH
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            $resultsPath = "$RepoRoot\runs\train\$runName\results.csv"
            if (Test-Path $resultsPath) {
                Write-Host "Checking final mAP@0.5 results..." -ForegroundColor Yellow
                Write-Host "Check results.csv for final mAP@0.5 performance" -ForegroundColor Green
            }
        } else {
