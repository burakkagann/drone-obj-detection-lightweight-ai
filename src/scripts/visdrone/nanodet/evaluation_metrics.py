...
# Optional import for hardware monitoring
try:
    import psutil
...
class NanoDetEvaluator:
    ...
    def evaluate_hardware_usage(self) -> Dict[str, Any]:
        ...
        if PSUTIL_AVAILABLE:
            ...
            # Fallback hardware metrics when psutil not available
            self.logger.warning("psutil not available - using fallback hardware metrics")
            hardware_metrics.update({
                'cpu_usage_percent': 0.0,
                'total_memory_gb': 8.0,  # Assume 8GB default
                'available_memory_gb': 4.0,  # Assume 4GB available
                'used_memory_gb': 4.0,  # Assume 4GB used
                'memory_usage_percent': 50.0  # Assume 50% usage
            })
        ...
        self.logger.info(f"  CPU usage: {hardware_metrics['cpu_usage_percent']:.1f}%")
        self.logger.info(f"  Memory usage: {hardware_metrics['memory_usage_percent']:.1f}%")
